...
const UI = (() => {
  ...
  function _renderMorningView() {
    ...
    const avoidEl  = document.getElementById('field-avoiding');
    const quotesEl = document.getElementById('field-quotes');

    if (avoidEl  && avoidEl.value  === '') avoidEl.value  = Storage.getAvoiding();
    if (quotesEl && quotesEl.value === '') quotesEl.value = Storage.getQuotes();
  }

  // ─────────────────────────────────────────────────────────
  // MORNING FIELD AUTO-SAVE
  // ─────────────────────────────────────────────────────────

  function _bindMorningFields() {
    _bindAutoSave('field-avoiding', val => Storage.setAvoiding(val), 'save-avoiding');
    _bindAutoSave('field-quotes',   val => Storage.setQuotes(val),   'save-quotes');
  }

  function _bindAutoSave(fieldId, saveFn, indicatorId) {
    ...
    field.addEventListener('input', () => {
      ...
      _autoSaveTimer = setTimeout(() => {
        saveFn(field.value);
        _flashSaveIndicator(indicator);
      }, 800);
    });

    // Also save immediately on blur (tab away / close keyboard)
  ...
  function _flashSaveIndicator(el) {
    if (!el) return;
  ...
  function _renderReviewView() {
    ...
    // ── Habits summary ─────────────────────────────────────
    const habitSummary = typeof Habits !== 'undefined'
      ? Habits.getTodaySummary()
      : _buildHabitSummaryFromStorage();
  ...
  function _buildHabitSummaryFromStorage() {
    ...
    return {
      ...
      total: names.length,
      pct:   names.length > 0
        ? Math.round((checked / names.length) * 100)
        : 0
    };
  }
  ...
  return {
    ...
    renderMorning:      _renderMorningView
  };
